...
- opt-in guided review defaults to one inbox item per confirmed request; an explicit exact-duplicate reach may apply the same action to the displayed count of eligible copies, while Teach Cortex is separately required before a review becomes policy evidence;
...
These rows are sensitive behavioral metadata. They remain in the local Cortex database, follow the database's existing backup and retention policy, disappear when their referenced memory is hard-deleted, and are only exposed through the authenticated dashboard snapshot. Shadow mode records them without changing the agent's prompt; enforcement is opt-in.
...
Memory Refinery data is private memory-derived content. `memory_presentations` rows contain derived titles and summaries built deterministically from stored memory text, and `memory_refinery_proposals` rows preserve operator-authored rewrite/split text, source memory IDs, before-state, and undo information. Both stay in the local database behind the authenticated dashboard. The refinery summary, dry-run classification report, and shadow retrieval comparison expose aggregate counts, flags, versions, and ID prefixes only — never memory content or source paths — and reference-card titles use note titles and section headings while full source paths appear only in the authenticated detail surface. Do not copy presentations, proposals, or refinery item listings into fixtures, screenshots, commits, or public reports.

Operator review decisions may contain an optional action-specific reason or free-text explanation in addition to decision reach, affected memory IDs, before-state, and applied effect. A neutral operator-choice code is stored when the operator skips the optional reason. These records remain in the local database and authenticated dashboard. One-off and exact-duplicate records remain auditable but are excluded from policy compilation; only explicit **Teach Cortex from this** decisions contribute policy evidence. Connection approval stores the operator decision as link evidence. Trash is a reversible tombstone, not content erasure; use a separately reviewed database-retention process if permanent deletion is legally or operationally required.
