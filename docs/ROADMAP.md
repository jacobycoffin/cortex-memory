...
| Creation Trainer and recall sets | Implemented (schema 23): automatic and agent-selected writes stage outside recall; operator decisions create primary or lookup-only records; a behavior-preserving Legacy set and reversible Trained set enforce eligibility across lexical, feature, context, graph, and archived-fallback paths. | Pending candidates never enter retrieval; duplicate legacy approval promotes without copying; activation changes no memory content; reopening cannot leak Legacy into Trained; switching back restores the prior set. |
