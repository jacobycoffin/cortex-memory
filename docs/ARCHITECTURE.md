...
`operator_review_decisions` is an append-only review ledger for these choices. It stores the item key, action, reason code, optional explanation, actor, before-state, applied effect, typed signal, and explicit `decision_scope`. The UI asks for the action first and treats reason as optional: an action-specific suggestion, free-form note, or neutral `operator_choice` can be stored. `item_only` mutates only the current review target; `exact_duplicates` can extend a single-memory action to eligible active/cold copies with the same normalized content, context, and source identity; neither scope enters policy compilation. Only `policy_evidence`, selected in the UI as **Teach Cortex from this**, is aggregated into pruning, linking, retrieval, or admission standards. A single decision never changes a global threshold or policy automatically.

Schema 20 adds `review_copilot_interpretations`, a non-recallable audit ledger for bounded natural-language review assistance. It stores the operator's words, prior copilot conversation, provider/model identifier, validated response, token usage when reported, and the review ID eventually confirmed. The LLM may return one clarifying question or a structured recommendation. The server restricts actions and reasons to the active proposal, forces non-explicit generalizations back to item-only reach, computes effect language deterministically, and rejects a confirmation whose action, reason, or scope changed after the preview. Only the existing review endpoint mutates memory or policy evidence.
