...
- added bounded user-visible memory receipts: Kaya lists up to three current-turn Cortex IDs only when they materially influenced an answer; an optional HTTPS Brain URL makes each ID a direct authenticated memory-detail link; Hermes now conservatively enforces a missing receipt from strong answer evidence instead of relying only on model compliance, while receipts remain excluded from capture and semantic replay and allow one-memory wrong or irrelevant feedback without penalizing unrelated recalled evidence;
