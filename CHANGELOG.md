...
- separated the low-frequency Brain Mechanics model and timeout from the frequent Auto-Judge provider settings so structured consolidation, pruning, reconsolidation, schema, and weight proposals can use a slower model without widening the normal review path;
