...
- added bounded user-visible memory receipts: Kaya lists up to three current-turn Cortex IDs only when they materially influenced an answer; receipts are excluded from capture and semantic replay, improve exact use attribution, and allow one-memory wrong or irrelevant feedback without penalizing unrelated recalled evidence;
