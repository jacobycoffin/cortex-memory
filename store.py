...
# Stage 1 of the store split (`docs/STORE_SPLIT_PLAN.md`): the schema and
# migration cluster now lives in `cortex_schema`. Its names are re-exported
# here so `from cortex.store import ...` keeps working, and `CortexStore`
# below keeps facades for the instance-level entry points.
...
class CortexStore:
    ...
    # Stage 1: the writer lives in cortex_schema; the class keeps the same
    # static entry point, so every `self._index_features_tx(...)` call site
    # (and `CortexStore._index_features_tx`) is untouched.
    ...
    # Stage 1: the writer lives in cortex_schema; the class keeps the same
    # static entry point, so every `self._index_context_terms_tx(...)` call
    # site (and `CortexStore._index_context_terms_tx`) is untouched.
    _index_context_terms_tx = staticmethod(_index_context_terms_tx)
